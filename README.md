...
<p align="center">
  <img src="docs/assets/animation/frame-touch.webp" alt="An isometric rotating display with an edge reader and a blue rotation bubble on screen" width="720">
</p>
...
Swivel is a small desktop utility for rotating a supported display from a bubble beside its fingerprint reader.

> [!CAUTION]
> Swivel is an unsigned prototype. System security may warn, managed-device policy may block it, and fingerprint behavior still needs to be verified on the target hardware.
...
The device must run a desktop edition that supports classic applications and its fingerprint reader.

1. [Download the compact `Swivel.exe`](https://github.com/jelizarovas/swivel/releases/latest/download/Swivel.exe) into a permanent local folder, such as `Documents\Swivel`.
2. Double-click it. The compact build is about 317 KB and uses the .NET 8 Desktop Runtime. If that runtime is missing, the system displays the required framework and a download link; install it, then open Swivel again.
...
7. Press the blue **Swivel** button only when ready to test the display orientation.
8. Choose **Right side down** or **Left side down** to match the physical mount.
...
- Device testing found the original right-side-down mapping was reversed; v0.1.2 corrected it. The corrected mapping still needs a device re-test.
- Not yet confirmed: the target reader's unclaimed-event support, corrected live rotation direction, high-DPI placement, lock/unlock behavior, sleep/resume behavior, and launch-at-sign-in on the target device.
